...
use axiomsync_domain::domain::{
    ActorRow, AnchorRow, ArtifactRow, EntryRow, IngressReceiptRow, ProjectionPlan, RawArtifactInput,
    SessionRow, empty_object, stable_id,
};
...
pub fn plan_projection(receipts: &[IngressReceiptRow]) -> Result<ProjectionPlan> {
    ...
    for receipt in ordered {
        let payload: Value = serde_json::from_str(&receipt.payload_json)?;
        ...
        let title = payload_string(
            &payload,
            &["title", "session_title", "thread_title", "summary", "name"],
        )
        .or_else(|| pointer_string(&payload, &["/thread/title", "/session/title"]));
        let session = session_map.entry(session_id.clone()).or_insert_with(|| SessionRow {
        ...
        if session.opened_at.as_ref().is_none_or(|opened| receipt.observed_at < *opened) {
            session.opened_at = Some(receipt.observed_at.clone());
        }
        if session.closed_at.as_ref().is_none_or(|closed| receipt.observed_at > *closed) {
            session.closed_at = Some(receipt.observed_at.clone());
        }
        if session.title.is_none() {
            session.title = payload_string(&payload, &["title", "summary"]);
        }

        let actor_kind = derive_actor_kind(&receipt.event_kind, &payload);
        ...
        actor_map.entry(actor_id.clone()).or_insert_with(|| ActorRow {
            actor_id: actor_id.clone(),
            actor_kind: actor_kind.clone(),
            stable_key: Some(stable_id("actor_key", &(receipt.connector.as_str(), actor_kind.as_str()))),
        ...
        entries.push(EntryRow {
            ...
            session_id: session_id.clone(),
            seq_no: *seq_no,
            entry_kind: receipt.event_kind.clone(),
            ...
            ended_at: receipt.captured_at.clone().or_else(|| Some(receipt.observed_at.clone())),
            metadata_json: empty_object(),
        });
        ...
        if let Some(preview_text) = text_body.clone().filter(|value| !value.trim().is_empty()) {
            anchors.push(AnchorRow {
                ...
                artifact_id: None,
                anchor_kind: "text_span".to_string(),
                locator_json: serde_json::json!({ "kind": "entry_text" }).to_string(),
                fingerprint: Some(stable_id("fingerprint", &(preview_text.as_str(), receipt.content_hash.as_str()))),
        ...
        for artifact_input in artifact_inputs {
            let artifact_id = stable_id(
                "artifact",
                &(session_id.as_str(), entry_id.as_str(), artifact_input.uri.as_str()),
            );
            ...
            anchors.push(AnchorRow {
                ...
                locator_json: serde_json::json!({ "kind": "artifact_uri", "uri": artifact_input.uri }).to_string(),
                preview_text: None,
...
fn derive_actor_kind(event_kind: &str, payload: &Value) -> String {
    payload_string(payload, &["actor", "role", "author_role"])
        .or_else(|| pointer_string(payload, &["/actor/kind", "/actor/role", "/role"]))
        .unwrap_or_else(|| {
