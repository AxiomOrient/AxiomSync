...
impl McpResourcePort for AxiomSync {
    ...
    fn read_mcp_resource(&self, uri: &str) -> crate::Result<Value> {
        ...
        if let Some(id) = uri.strip_prefix("axiom://procedures/") {
        ...
        Err(crate::AxiomError::Validation(format!("unknown resource {uri}")))
    }
...
impl McpToolPort for AxiomSync {
    ...
    fn call_mcp_tool(
        ...
        match name {
            "search_entries" => Ok(serde_json::to_value(self.search_entries(serde_json::from_value(arguments.clone())?)?)?),
            "search_episodes" => Ok(serde_json::to_value(self.search_episodes(serde_json::from_value(arguments.clone())?)?)?),
            "search_claims" => Ok(serde_json::to_value(self.search_claims(serde_json::from_value(arguments.clone())?)?)?),
            "search_procedures" => Ok(serde_json::to_value(self.search_procedures(serde_json::from_value(arguments.clone())?)?)?),
            ...
            "get_artifact" => Ok(serde_json::to_value(self.get_artifact(id_arg(arguments)?)?)?),
            ...
            _ => Err(crate::AxiomError::Validation(format!("unknown tool {name}"))),
        }
    }

    fn tool_workspace_requirement(&self, name: &str, arguments: &Value) -> crate::Result<Option<String>> {
        match name {
