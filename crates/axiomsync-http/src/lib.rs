...
use axiomsync_domain::domain::{
    AppendRawEventsRequest, SearchClaimsRequest, SearchEntriesRequest, SearchEpisodesRequest,
...
async fn rebuild_projection(headers: HeaderMap, State(state): State<AppState>) -> HttpResult<Json<Value>> {
    authorize_admin(&state.app, &headers)?;
    let plan = state.app.build_projection_plan()?;
    Ok(Json(state.app.apply_projection_plan(&plan)?))
}

async fn rebuild_derivations(headers: HeaderMap, State(state): State<AppState>) -> HttpResult<Json<Value>> {
    authorize_admin(&state.app, &headers)?;
    let plan = state.app.build_derivation_plan()?;
    Ok(Json(state.app.apply_derivation_plan(&plan)?))
}

async fn rebuild_index(headers: HeaderMap, State(state): State<AppState>) -> HttpResult<Json<Value>> {
    authorize_admin(&state.app, &headers)?;
...
async fn get_session(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
...
async fn get_entry(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    let entry = state.app.get_entry(&id)?;
    authorize_workspace_resource(&state.app, &headers, state.app.session_workspace_id(&entry.session_id)?)?;
    Ok(Json(serde_json::to_value(entry)?))
}

async fn get_artifact(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
...
async fn get_anchor(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
...
async fn get_episode(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
...
async fn get_claim(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
...
async fn get_procedure(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_admin(&state.app, &headers)?;
    Ok(Json(serde_json::to_value(state.app.get_procedure(&id)?)?))
}

async fn search_entries(headers: HeaderMap, State(state): State<AppState>, payload: Json<SearchEntriesRequest>) -> HttpResult<Json<Value>> {
    authorize_workspace_filter(&state.app, &headers, payload.filter.workspace_root.as_deref())?;
    Ok(Json(serde_json::to_value(state.app.search_entries(payload.0)?)?))
}

async fn search_episodes(headers: HeaderMap, State(state): State<AppState>, payload: Json<SearchEpisodesRequest>) -> HttpResult<Json<Value>> {
    authorize_workspace_filter(&state.app, &headers, payload.filter.workspace_root.as_deref())?;
    Ok(Json(serde_json::to_value(state.app.search_episodes(payload.0)?)?))
}

async fn search_claims(headers: HeaderMap, State(state): State<AppState>, payload: Json<SearchClaimsRequest>) -> HttpResult<Json<Value>> {
    authorize_workspace_filter(&state.app, &headers, payload.filter.workspace_root.as_deref())?;
    Ok(Json(serde_json::to_value(state.app.search_claims(payload.0)?)?))
}

async fn search_procedures(headers: HeaderMap, State(state): State<AppState>, payload: Json<SearchProceduresRequest>) -> HttpResult<Json<Value>> {
    authorize_workspace_filter(&state.app, &headers, payload.filter.workspace_root.as_deref())?;
    Ok(Json(serde_json::to_value(state.app.search_procedures(payload.0)?)?))
}

async fn get_case(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_workspace_resource(&state.app, &headers, state.app.episode_workspace_id(&id)?)?;
    Ok(Json(serde_json::to_value(state.app.get_case(&id)?)?))
}

async fn get_thread(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_workspace_resource(&state.app, &headers, state.app.session_workspace_id(&id)?)?;
    Ok(Json(serde_json::to_value(state.app.get_thread(&id)?)?))
}

async fn get_runbook(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_workspace_resource(&state.app, &headers, state.app.episode_workspace_id(&id)?)?;
...
async fn get_run(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_workspace_resource(&state.app, &headers, state.app.session_workspace_id(&id)?)?;
    Ok(Json(serde_json::to_value(state.app.get_run(&id)?)?))
}

async fn get_task(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
...
async fn get_document(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_workspace_resource(&state.app, &headers, state.app.artifact_workspace_id(&id)?)?;
    Ok(Json(serde_json::to_value(state.app.get_document(&id)?)?))
}

async fn get_evidence(headers: HeaderMap, State(state): State<AppState>, Path(id): Path<String>) -> HttpResult<Json<Value>> {
    authorize_workspace_resource(&state.app, &headers, state.app.anchor_workspace_id(&id)?)?;
...
async fn mcp_http(
    ...
    Ok(Json(mcp::handle_request(&state.app, request, workspace_id.as_deref())?))
}
...
fn authorize_workspace_filter(app: &AxiomSync, headers: &HeaderMap, workspace_root: Option<&str>) -> Result<Option<String>> {
...
fn authorize_workspace_resource(app: &AxiomSync, headers: &HeaderMap, workspace_id: Option<String>) -> Result<Option<String>> {
    let token = bearer_token(headers)
        .ok_or_else(|| AxiomError::PermissionDenied("missing bearer token".to_string()))?;
