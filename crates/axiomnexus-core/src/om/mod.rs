...
// re-export pure types/transforms and keep only AxiomNexus-specific rollout/error helpers local.
mod failure;
mod rollout;
mod thread_identity;
...
pub use episodic::{
    ...
    DEFAULT_REFLECTOR_OBSERVATION_TOKENS, OM_PROMPT_CONTRACT_NAME, OM_PROMPT_CONTRACT_VERSION,
    OM_PROTOCOL_VERSION, OM_SEARCH_VISIBLE_SNAPSHOT_V2_VERSION, ObservationConfigInput,
    ObserverWriteDecision, OmApplyAddon, OmCommand, OmConfigError, OmConfigInput,
    OmContinuationCandidateV2, OmContinuationSourceKind, OmContinuationStateV2,
    OmDeterministicObserverResponseV2, OmHintPolicyV2, OmInferenceModelConfig, OmInferenceUsage,
    OmMemorySection, OmMultiThreadObserverAggregate, OmMultiThreadObserverSection,
    OmObservationChunk, OmObservationEntryV2, OmObservationOriginKind, OmObservationPriority,
    OmObserverAddon, OmObserverMessageCandidate, OmObserverPromptContractV2, OmObserverPromptInput,
    ...
    build_reflection_draft, build_reflector_prompt_contract_v2, build_reflector_system_prompt,
    build_reflector_user_prompt, build_scope_key, calculate_dynamic_threshold,
    combine_observations_for_buffering, compute_pending_tokens, decide_observer_write_action,
    decide_reflection_enqueue, evaluate_async_observation_interval, extract_list_items_only,
