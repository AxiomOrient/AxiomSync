...
pub const AXIOMNEXUS_HOST_TOOLS_ENV: &str = "AXIOMNEXUS_HOST_TOOLS";
...
pub fn resolve_host_tools_policy() -> HostToolsPolicy {
    let env_raw = std::env::var(AXIOMNEXUS_HOST_TOOLS_ENV).ok();
...
fn format_host_tools_feature_disabled_reason(operation: &str) -> String {
    format!(
        "host_tools_unavailable operation={operation} feature=host-tools (compile axiomnexus-core with feature `host-tools` to enable host process execution)"
    )
}
