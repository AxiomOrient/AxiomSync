...
mod tests {
    ...
    fn parse_llm_memories_accepts_embedded_json_content() {
        let payload = serde_json::json!({
            "message": {
                "content": "```json\n[{\"category\":\"entity\",\"text\":\"Project is AxiomNexus\",\"source_message_ids\":[\"m2\"]}]\n```"
            }
        });
        let messages = vec![msg("m2", "Project is AxiomNexus")];
        let parsed = parse_llm_memories(&payload, &messages).expect("parse");
        assert_eq!(parsed.len(), 1);
        assert_eq!(parsed[0].category, "entities");
        assert_eq!(parsed[0].text, "Project is AxiomNexus");
    }
