...
const RELATION_TRACE_LOGS_PATH: &str =
    "crates/axiomnexus-core/src/client/tests/relation_trace_logs.rs";
...
pub(super) fn run_contract_execution_probe(workspace_dir: &Path) -> CommandProbeResult {
    ...
    if !core_crate.exists() {
        return CommandProbeResult::from_error(
            ...
            "missing_axiomnexus_core_crate".to_string(),
        );
    }
...
fn load_bootstrapped_ontology_schema(
    ...
    let probe_root = std::env::temp_dir().join(format!(
        "axiomnexus-ontology-contract-probe-{}",
        uuid::Uuid::new_v4().simple()
    ));
    let app = crate::AxiomNexus::new(&probe_root)
...
fn resolve_prompt_signature_baseline_revision(
    ...
    if let Ok(explicit_base_revision) = std::env::var("AXIOMNEXUS_PROMPT_SIGNATURE_BASE_REV") {
