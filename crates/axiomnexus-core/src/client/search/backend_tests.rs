...
use super::{
    AxiomNexus, OmHintPolicy, merge_observation_hint_with_suggested_response,
...
fn setup_test_app() -> (TempDir, AxiomNexus) {
    let temp = tempdir().expect("tempdir");
    let app = AxiomNexus::new(temp.path()).expect("app");
    app.initialize().expect("init");
    (temp, app)
...
fn upsert_records(app: &AxiomNexus, records: &[IndexRecord]) {
    {
        let mut index = app.index.write().expect("index write");
