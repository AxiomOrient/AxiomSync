...
fn parse_llm_observer_response_accepts_embedded_json_content() {
    let payload = serde_json::json!({
        "message": {
            "content": "```json\n{\"header\":{\"contract_name\":\"axiomsync.om.prompt\",\"contract_version\":\"2.0.0\",\"protocol_version\":\"om-v2\"},\"observations\":\"[assistant] keep signal\",\"observed_message_ids\":[\"m9\"]}\n```"
        }
    });
...
fn parse_llm_observer_response_rejects_marker_like_plain_text_without_structured_contract() {
    let payload = serde_json::json!({
        "message": {
            "content": "notes: contract_name axiomsync.om.prompt contract_version 2.0.0 protocol_version om-v2\n- line-a\n- line-b"
        }
    });
    let known_ids = vec!["m7".to_string()];
    let err = parse_llm_observer_response(
...
fn deterministic_fallback_emits_current_task() {
    ...
    assert_eq!(
        ...
        Some(
            "Primary: Please fix AXIOMSYNC_RERANKER handling in src/client/search/mod.rs and verify tests."
        )
    );
    assert!(
        response
            .suggested_response
            .as_deref()
            .is_some_and(|value| value.contains("AXIOMSYNC_RERANKER")),
...
fn deterministic_fallback_populates_thread_states_for_resource_scope() {
    let selected = vec![
        msg_with_thread(
            "m1",
            "user",
            "Please fix queue replay for worker-a and verify AXIOMSYNC_RERANKER stability.",
            "thread-a",
            "s-fallback-threads",
        ),
        msg_with_thread(
            "m2",
            "tool",
            "error E401 at worker-a while applying AXIOMSYNC_RERANKER patch",
            "thread-a",
            "s-fallback-threads",
        ),
        msg_with_thread(
...
fn deterministic_fallback_extracts_identifiers_without_whitespace() {
    let selected = vec![
        msg(
            "m1",
            "user",
            "Please fix queue replay handling in worker:queue and keep AXIOMSYNC_RERANKER stable.",
        ),
