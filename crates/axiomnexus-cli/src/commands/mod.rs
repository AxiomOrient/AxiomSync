...
use axiomnexus_core::markdown_preview::render_markdown_html as render_preview_html;
...
pub(crate) fn run_from_root(root: &Path, command: Commands) -> Result<()> {
    ...
    let app = AxiomNexus::new(root).context("failed to create app")?;
    run_validated(&app, root, command)
}

fn run_validated(app: &AxiomNexus, root: &Path, command: Commands) -> Result<()> {
