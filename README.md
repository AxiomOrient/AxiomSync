...
# 리소스 추가
cargo run -p axiomme-cli -- add ./docs --target axiom://resources/docs

# 검색
cargo run -p axiomme-cli -- search "oauth flow" --target axiom://resources/docs
```
...
# 로컬 binary 설치(선택)
cargo install --path crates/axiomme-cli
```
...
```bash
# 1) 세션 생성
SID="$(cargo run -q -p axiomme-cli -- session create)"

# 2) 대화 메시지
cargo run -p axiomme-cli -- session add --id "$SID" --role user --text "배포 전 검증 시작"

# 3) 대화 외 이벤트(예: 테스트 결과)
cargo run -p axiomme-cli -- session add --id "$SID" --role tool --text "integration: 24 passed, 1 flaky"

# 4) 세션 문맥을 검색에 반영
cargo run -p axiomme-cli -- search "flaky 원인" --session "$SID"

# 5) 세션 커밋(아카이브 + memory 추출 흐름)
cargo run -p axiomme-cli -- session commit --id "$SID"
```
